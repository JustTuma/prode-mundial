...
export default async function GoleadoresPage() {
  ...
  const { data: scorers } = await supabase.from('scorers').select('*').order('position').limit(30)
          ...
          {scorers.map((s: any, i: number) => {
            ...
            return (
              <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 0', borderBottom: i < scorers.length - 1 ? '1px solid var(--line)' : 'none' }}>
                ...
                <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: teamColor(code), color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Anton', fontSize: '11px', flexShrink: 0 }}>{code}</div>
